...
def find_field_of_view(img8, bright_thresh=12, shrink_frac=0.16, min_keep_frac=0.2):
    ...
    if fills_workspace:
        # Already a tight crop with no vignette -- don't nibble a stray edge
        # pixel off an otherwise-clean image.
...
def extract_candidates(mask, flat, vesselness, min_area_px=40):
    ...
    # including tiny noise specks that get discarded below by the min-area
    # check -- so a surviving candidate can keep an arbitrary, non-sequential
    # ID (e.g. "9") purely because 8 smaller regions before it in scan order
    # happened to get filtered out. Surviving candidates are renumbered
    # 1..N (in the same scan order) below, and `labeled` is remapped to
    # match, so the Label a user sees always reflects "the Nth real
    # candidate," not a leftover ID from regions that no longer exist.
        ...
        records.append(
            {
                ...
                "AreaCoveragePct": 100.0 * p.area / (mask.shape[0] * mask.shape[1]),
                "HWRatio": (y1 - y0) / (x1 - x0),
            }
        )
...
def classify_with_model(df, model_path, proba_threshold=0.5, force_keep_area=50000):
    import joblib
...
def main():
    ...
    parser.add_argument("--proba-threshold", type=float, default=0.5, help="Probability threshold in --mode apply.")
